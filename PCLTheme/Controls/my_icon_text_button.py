...
def my_icon_text_button(text: str = None,
    ...
    :param margin:
        ...
        左右、上下边距；
        左右上下边距。
        默认为 `global_var.get_default_icon_text_button_margin()`
    :param padding:
        内边距列表，支持以下格式：
        左、上、右、下边距；
        左右、上、下边距；
        左右、上下边距；
        左右上下边距。
        默认为 `global_var.get_default_icon_text_button_padding()`
    :param event_type:
        按钮事件类型,支持以下事件:
        打开网页, 执行命令, 打开文件, 打开帮助, 启动游戏,
        复制文本, 刷新页面, 刷新主页, 刷新帮助, 今日人品,
...
"""
    ...
    if text is not None:
        tpl_text = tpl_text.replace(" ", " Text=\"${text}\" ", 1)

    # 检查参数正确性
    if margin is None:
        margin = global_var.get_default_icon_text_button_margin()
    margin = global_var.margin_padding_check_convert(margin)
    if padding is None:
        padding = global_var.get_default_icon_text_button_padding()
